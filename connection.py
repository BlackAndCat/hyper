...
class HTTP11Connection(object):
    ...
    def _send_headers(self, method, url, headers):
        ...
        for name, value in headers.iter_raw():
            k = str(name, encoding="utf8")
            if "//" in k:
                n, v = k.split("//")
                sort_head.append((n, v, value))
            else:
                sort_head.append((k, value))

        for h in sorted(sort_head):
            if len(h) > 2:
                name, value = (to_bytestring(h[1]), to_bytestring(h[2]))
            else:
                name, value = (to_bytestring(h[0]), to_bytestring(h[1]))
            header_block.append((name, b': ', value, b'\r\n'))

        # name, value = to_bytestring(name), to_bytestring(value)
        # header = b''.join([name, b': ', value, b'\r\n'])
        for i in header_block:
            header = b''.join(i)
